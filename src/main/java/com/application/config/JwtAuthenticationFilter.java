...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private boolean isPermittedRoute(String requestURI) {
        for (String permittedRoute : SecurityConfiguration.PERMITTED_ROUTES) {
            if (matchesPermittedRoute(requestURI, permittedRoute)) {
                return true;
            }
        }
        return false;
    ...
    private boolean matchesPermittedRoute(String requestURI, String permittedRoute) {
        if (permittedRoute.endsWith("/**")) {
            String modifiedRoute = permittedRoute.substring(0, permittedRoute.length() - 3);
            return requestURI.startsWith(modifiedRoute);
        } else {
            return requestURI.equals(permittedRoute) || requestURI.startsWith(permittedRoute + "/");
        }
    }
